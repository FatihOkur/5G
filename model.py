...
def tune_and_compare_models(featured_data_path):
    """
    Loads data, tunes, trains, and compares RF and XGBoost models.
    """
    # --- 1. Load Data ---
    try:
    ...
    rf_search = RandomizedSearchCV(estimator=rf, param_distributions=rf_param_grid, n_iter=20, cv=3, verbose=2, random_state=42, n_jobs=-1)
    ...
    xgb_search = RandomizedSearchCV(estimator=xgbr, param_distributions=xgb_param_grid, n_iter=20, cv=3, verbose=2, random_state=42, n_jobs=-1)
    ...
    for name, search_cv in models.items():
        ...
        results[name] = {'model': best_model, 'mae': mae, 'mean_error': mean_error, 'predictions': y_pred}
    ...
    print(f"\n🏆 Best performing model: {best_model_name} with {best_model_info['mean_error']:.2f}m error.")
    ...
    plt.title(f'Actual vs. Predicted Location ({best_model_name})')
    plt.xlabel('Longitude')
    plt.ylabel('Latitude')
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(visuals_dir, f'best_model_predictions_{best_model_name}.png'))
    plt.show()
    ...
    model_path = os.path.join(models_dir, 'best_positioning_model.joblib')
...
if __name__ == '__main__':
    FEATURED_DATA_PATH = os.path.join("Data", "featured_data.xlsx")
    tune_and_compare_models(FEATURED_DATA_PATH)
